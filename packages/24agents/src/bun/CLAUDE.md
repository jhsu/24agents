...
- `server.ts` runs a Bun HTTP server on port 4000 with the following endpoints:
  ...
  - `POST /api/chat/explore` - **Primary exploration endpoint.** Accepts `{ prompt, history?, systemPrompt?, sessionId? }`. Asks Claude to return structured JSON with 3-4 named sections, 2-3 branching paths with per-branch CFNR scores, and an overall `promptScore`. Returns `{ sections: [{id, title, content}], branches: [{id, label, description, previewScore}], promptScore: {C,F,N,R} | null }`. Enriches with memory context when sessionId is provided.
  - `POST /api/chat/score-prompt` - Lightweight prompt scoring endpoint. Accepts `{ prompt, systemPrompt? }`. Returns `{ score: {C,F,N,R} }`. Uses 256 max_tokens for fast turnaround.
  ...
  - `POST /api/chat/rewrite` - Prompt rewrite endpoint. Accepts `{ prompt, personaPrompt, iterationContext?, context?, goals? }`. Asks Claude to rewrite the prompt from the persona's perspective, including user-provided context and goals in the prompt when present, and return `{ refinedPrompt, responseText, score: { C, F, N, R } }`. Falls back to original prompt on parse failure.
