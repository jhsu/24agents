...
- `server.ts` runs a Bun HTTP server on port 4000 with the following endpoints:
  ...
  - `POST /api/chat/explore` - **Primary exploration endpoint.** Accepts `{ prompt, history?, systemPrompt?, sessionId? }`. Asks Claude to return structured JSON with 3-4 named sections and 2-3 branching paths in a single non-streaming call. Returns `{ sections: [{id, title, content}], branches: [{id, label, description}] }`. Enriches with memory context when sessionId is provided.
