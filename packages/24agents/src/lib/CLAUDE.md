...
- `exploration.ts` - **Primary data model.** Defines `ExploreSection` (id, title, content), `ExploreBranch` (id, label, description, previewScore), `ExploreStep` (prompt + sections + branches + parent link + score + refinements + activeRefinementId), `PromptRefinement` (id, personaId, personaName, refinedPrompt, reasoning, score, createdAt), `PendingExploration` (originalPrompt, refinements, activeRefinementId, parentStepId), `ExploreSession`, `ExploreSessionListEntry`. Pure helpers: `createExploreSession()`, `createStep()`, `addStepToSession()`. Persistence via localStorage at `"24agents:explore:{id}"` per session and `"24agents:explore-list"` for the index.
- `sse-client.ts` - API client for the backend. `explorePrompt()` calls `POST /api/chat/explore` and returns `{ sections, branches, promptScore }`. `scorePrompt()` calls `POST /api/chat/score-prompt` for lightweight CFNR scoring. `streamChat()` is an async generator that consumes SSE from `POST /api/chat`. `fetchBranches()` calls `POST /api/chat/branches`. `rewritePrompt()` calls `POST /api/chat/rewrite`. `fetchPersonaPaths()` calls `POST /api/chat/persona-paths`. `persistToMemory()` and `searchMemory()` handle memory server calls.
- `iteration.ts` - Shared types used by both exploration and legacy systems. Defines `IterationScore` (CFNR scores), `PersonaPath`, `scoreColor()`. Also contains legacy types: `Iteration`, `PromptSession`, `PromptInput`, `SessionListEntry`.
