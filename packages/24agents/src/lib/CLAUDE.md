...
- `sse-client.ts` - API client for the backend. `streamChat()` is an async generator that consumes SSE from `POST /api/chat`. `fetchBranches()` calls `POST /api/chat/branches` and returns parsed `BranchSuggestion[]`.
