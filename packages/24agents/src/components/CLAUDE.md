...
- `ExploreView.tsx` - **Main app view.** Unified exploration interface with inline refinement. Three rendering states: normal (SectionPanel left + branch cards right), refining (RefinementPanel), history. Branch cards show CFNR score previews and dual "Refine"/"Explore" buttons. Prompt bar has "Refine" (sparkle icon) and "Send" buttons. Uses `useExploration` hook. Layout is a flex column constrained to `h-screen`; the content area uses `overflow-hidden` to stay within its flex allocation so the prompt bar remains pinned at the bottom. Exploration path (bottom-right) shows clickable step breadcrumbs — clicking a past step calls `navigateToStep()` to revert the view.
- `SectionPanel.tsx` - Left panel showing stacked section cards grouped by exploration step. Each step group has a numbered indicator, step prompt label, and optional CFNR score badges (via `ScoreRow`). Cards show section title + markdown content, collapsible for long content (>500 chars). Includes simple inline markdown rendering with bold, italic, code, and links. Markdown links (`[text](url)`) open in the system browser via `window.open()`.
